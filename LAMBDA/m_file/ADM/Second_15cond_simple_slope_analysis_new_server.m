%% (Part9-b)Simple Slope Analysis(pxm interaction)-->Extrac Beta(conditional:15 cond.) from PxM peak(parametric)
...
betatype={
    ...
    'LLH' 'LLM' 'LLL'};
all_peak={};
%Specify the pxm peaks location
for v=3:6;%Sepcify the v2p peaks
    for c_index=3;
peak=[];
C=read_mixed_csv(strcat(path,'/Second_ROI_defined_by_value2prob_new_server/normalise_3x3x3/two_SVS_in_same_GLM/',...
                    cond{c_index},'/result_table_p005_size15/',value{v},'.csv'),',');
peak=str2double(C(3:end,[12:14])); %read : the peak size,p-value,peak locations
all_peak{c_index,v}=peak; 
    end
end
%% Extract 15 cond. Betas from the peaks
beta_peak_value_cond_roi={};
con_num_index=strcat(num2str(0),num2str([1:9]'));
con_num_index=cellstr([con_num_index;num2str([10:15]')]);
...
%Save as nii. for display to check if the mask is right-->Correct
% %  tabulate(Y_same_size(:))
%  vol=V_mask;
%  vol.fname='/bml/Data/Bank6/ADM-YunShiuan/smooth_mask_mni_grey_white.nii';
%  %convert Y_mni_mask to matrix form(voxle-based coordinates)
%  img=NaN([121,145,121]);
%  Y_cor_mask=mni2cor([round(Y_mni_mask(:,1)) ,round(Y_mni_mask(:,2)), round(Y_mni_mask(:,3))],T_mask);
%  img(sub2ind([121,145,121],Y_cor_mask(:,1) ,Y_cor_mask(:,2),Y_cor_mask(:,3)))=1;
...
for c_index=3; %specify condition(pxm peaks location)
    for beta= 1:15; % 15 conditions
        [imgfilename,dirs]=spm_select('ExtFPListRec',...%FORMAT [files,dirs] = spm_select('ExtFPListRec',direc,filt,frames)
            strcat(path,'/First_design_matrix_with_contrastncovariate_categorical/'),...
            strcat('.*con_00',con_num_index(beta),'.nii'),1); 
            %get the interested row indexes of those with normarlise_3x3x3
            ...
            imgfilename=imgfilename([1:8,10:16,18:size(imgfilename,1)],:); %exclude S031,S040(S44 has already been skipped in the first level)          
...
%                  vol=V_mask;
%                  vol.fname='/bml/Data/Bank6/ADM-YunShiuan/roi_54_32_23.nii';
%                  %convert Y_mni_mask to matrix form(voxle-based coordinates)
%                  img=NaN([121,145,121]);
...
%             %(Unused:marsbar script)roi_mni_mask=marsbar('on');
%             my_centers = [1 2 3];
%             radius = 5;
%             rois = {}
%             params = struct('centre', my_centers, 'radius', radius)
...
cd(strcat(path,'/Second_ROI_defined_by_value2prob_new_server/normalise_3x3x3/two_SVS_in_same_GLM/Betas_of_15cond_extracted_from_pm_peaks_overlaid'))
for c_index=3;
    for v=3:6;
       for beta=1:15; % 15 conditions
        for p=1:size(all_peak{c_index,v},1); %specify which peak in the results table 
            ...
            '_z',num2str(all_peak{c_index,v}(p,end)),'.csv'),...
              [beta_peak_value_cond_roi{p,v,c_index,beta},id],...%Cbind ID info
          'precision',9)  
            end
        v
        end
...
cd(strcat(path,'/Second_ROI_defined_by_value2prob_new_server/normalise_3x3x3/two_SVS_in_same_GLM/Betas_of_15cond_extracted_from_pm_peaks_overlaid/ROI_overlaid_nii_file'))
for c_index=3; %specify condition(pxm peaks location)
    for beta= 1:15; % 15 conditions
           for v=3:6; %specify v2p peaks      
        for p=1:size(all_peak{c_index,v},1); %specify which peak in the results table
...
%         %Y_mni_mask %the mask of grey and white matter;
              ...
              %Save as nii. for display to check if the ROI_overlaid is right-->Correct
                 vol=V_mask;
                 file_name=char(strcat(path,'/Second_ROI_defined_by_value2prob_new_server/normalise_3x3x3/two_SVS_in_same_GLM/Betas_of_15cond_extracted_from_pm_peaks_overlaid/ROI_overlaid_nii_file/roi_',...
                                num2str(center),'.nii'));
                 vol.fname=file_name;
                 %convert Y_mni_mask to matrix form(voxle-based coordinates)
                 img=NaN([121,145,121]);
 ...
 %% Get the HRF tranformed Mag./Prob. Level (H/M/L)--> NO NEED!!!(Since HRF(cxProb)=cxHRF(Prob), no need to calculate HRF(c))
% path='/bml/Data/Bank6/ADM-YunShiuan';
