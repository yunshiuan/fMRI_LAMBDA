%% Part 2-a-ii - Second Level (Parametric)- Split age group: Seperate GLM for 2nd grader and 5th grader
...
T_Parameters_name={'B0_LL' 'B0_LL' 'B0_LF' 'B0_FF'... 
                   'Dist_All'...
                   'Dist_LL' 'Dist_LF' 'Dist_FF'... 
                   't_LF-LL' 't_LL-LF' ...
                   't_FF-LL' 't_LL-FF' ...
                   't_FF-LF' 't_LF-FF'};
age_Group={'Second_grader' 'Fifth_grader'};
...
% Grade index per person as 2nd or 5th grader (used in the loop below)
grade_index={valid_Grade==2,valid_Grade==5};
...
for con=1:size(T_Parameters_name,2) %NOTE: con=1 refers to spmT_0002
    for ageGroup=1:2 %NOTE: ageGroup=1 refers to second graders 
    matlabbatch='';
    second_Result_Dir = strcat('D:\Yun-Shiuan_LAMBDA\Second_level_parametric_new_split_age\',age_Group{ageGroup},'\',T_Parameters_name{con});
    ...
    first_T_Contrast=strcat('D:\Yun-Shiuan_LAMBDA\First_level_parametric_design_matrix_and_result\',...
                             subject_list(grade_index{ageGroup}),'\spmT_',numeric_Contrast_Index(con),'.nii,1');
    matlabbatch{1}.spm.stats.factorial_design.des.t1.scans = first_T_Contrast;
    % Covariate
    % Age
    matlabbatch{1}.spm.stats.factorial_design.cov(1).c = valid_Age_Precise(grade_index{ageGroup});
    matlabbatch{1}.spm.stats.factorial_design.cov(1).cname = 'Age';
    matlabbatch{1}.spm.stats.factorial_design.cov(1).iCFI = 1;
    matlabbatch{1}.spm.stats.factorial_design.cov(1).iCC = 1;
    % Gender
    matlabbatch{1}.spm.stats.factorial_design.cov(2).c = valid_Gender_Dummy(grade_index{ageGroup});
...
for ageGroup=1:2
    for first_con=1:size(T_Parameters_name,2)
        matlabbatch='';
        spm_File=strcat('D:\Yun-Shiuan_LAMBDA\Second_level_parametric_new_split_age\',age_Group{ageGroup},'\',T_Parameters_name{first_con},'\SPM.mat');
        matlabbatch{1}.spm.stats.results.spmmat = {spm_File};

        for second_con=1:6
            threshold=(second_con<=2)*(0.005)+(second_con>=3)*(0.05); 
            % Set a higher p threshold for age and gender, 
        ...
        result_table_Dir=strcat('D:\Yun-Shiuan_LAMBDA\Second_level_parametric_new_split_age\',age_Group{ageGroup},'\',T_Parameters_name{first_con},'\Result_table');
        mkdir(result_table_Dir);
        csv_Old_Name=ls(strcat('D:\Yun-Shiuan_LAMBDA\Second_level_parametric_new_split_age\',age_Group{ageGroup},'\',T_Parameters_name{first_con},'\spm*.csv'));
        ...
        csv_Old_Full_Name=strcat('D:\Yun-Shiuan_LAMBDA\Second_level_parametric_new_split_age\',age_Group{ageGroup},'\',T_Parameters_name{first_con},'\',csv_Old_Name);
