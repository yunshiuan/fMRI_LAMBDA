%% Part0-a-ii: Create new GLMs(for distance effect DCM) - first level design matrix (EPI with reslice)
%% [Event-Related (Duration = 0s)] : To maximize the capibility to capture stumulation induced effect
%% Outline
...
%% Part0-a-i:  This GLM is for categorical 2nd level review usage (which each GLM includes 6 runs)
% % --------------------------------------------------------------------------
%spm_jobman('initcfg') -- before running the batch
path='D:\Yun-Shiuan_LAMBDA\First_level_parametric_design_matrix_and_result\with_reslice';
path_first='D:\Yun-Shiuan_LAMBDA\DCM\Part0_First_GLM_for_DCM_multiple_runs_run_with_reslice';
cd(path);
...
for i=1:size(subject_list,1)
    clear matlabbatch
    load('D:\Yun-Shiuan_LAMBDA\DCM\Part0_First_GLM_for_DCM_multiple_runs_run_with_reslice\template.mat');
    ...
    if(isnan(run_valid)==1)
        ...
        for run=1:length(run_valid)
            ...
            % Get all design matix variables: e.g., onset times & stimulus
            % duration
            ...
            Near_duration=run_info{strcmp(run_info.fraccomp_sti_dis_type,'Near') & row_non_dummy,'fraccomp_sti_duration'};
            Medium_duration=run_info{strcmp(run_info.fraccomp_sti_dis_type,'Medium') & row_non_dummy,'fraccomp_sti_duration'};
            Far_duration=run_info{strcmp(run_info.fraccomp_sti_dis_type,'Far') & row_non_dummy,'fraccomp_sti_duration'};
            ...
            matlabbatch{1}.spm.stats.fmri_spec.sess(batch_index).cond(1).duration = Near_duration;
            ...
            matlabbatch{1}.spm.stats.fmri_spec.sess(batch_index).cond(2).duration = Medium_duration;
            ...
            matlabbatch{1}.spm.stats.fmri_spec.sess(batch_index).cond(3).duration = Far_duration;
...
cd('D:\Yun-Shiuan_LAMBDA\First_level_parametric_design_matrix_and_result\with_reslice');
...
for i=1:length(subject_list)
    try
        load(strcat('D:\Yun-Shiuan_LAMBDA\DCM\Part0_First_GLM_for_DCM_multiple_runs_run_with_reslice\',...
...
% --------------------------------------------------------------------------
path='D:\Yun-Shiuan_LAMBDA\First_level_parametric_design_matrix_and_result\with_reslice';
path_first='D:\Yun-Shiuan_LAMBDA\DCM\Part0_First_GLM_for_DCM_single_run_with_reslice\full_duration';
cd(path);
subject_list=cellstr(ls('*')); % subject list
subject_list=subject_list(~cellfun(@isempty,regexp(subject_list,'(?<=df)\d+','match')));% include only subject name
subject_list=subject_list(~ismember(subject_list,{'df1001','df1054'})); %Exclude df1001(no T1) and df1054(incomplete)
...
for i=1:size(subject_list,1)
    ...
    for run=1:length(run_valid)
        r=run_valid(run);% The index refer to the real run number of the task (so it might not be consecutive due to exclusive runs)
        clear matlabbatch
        load('D:\Yun-Shiuan_LAMBDA\DCM\Part0_First_GLM_for_DCM_single_run_with_reslice\full_duration\template.mat');
        % Specify outputs of SPM.mat and all first level results
        ...
        if(isnan(run_valid)==1)
            ...
            matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).duration = Near_duration;
            ...
            matlabbatch{1}.spm.stats.fmri_spec.sess.cond(2).duration = Medium_duration;
            
            matlabbatch{1}.spm.stats.fmri_spec.sess.cond(3).name = 'Far';
            matlabbatch{1}.spm.stats.fmri_spec.sess.cond(3).onset = Far_onset;      
            matlabbatch{1}.spm.stats.fmri_spec.sess.cond(3).duration = Far_duration;
            ...
            motion_parameters_file=strcat('D:\Yun-Shiuan_LAMBDA\preprocessed_data_with_reslice_trial2\',...
                subject_list{i},'\rp_run_',...
                num2str(r),'.txt');
...
for i=1:length(subject_list)
    ...
    for run=1:length(run_valid)
        r=run_valid(run);% The index refer to the real run number of the task (so it might not be consecutive due to exclusive runs)
        try
            load(strcat('D:\Yun-Shiuan_LAMBDA\DCM\Part0_First_GLM_for_DCM_single_run_with_reslice\full_duration\',...
