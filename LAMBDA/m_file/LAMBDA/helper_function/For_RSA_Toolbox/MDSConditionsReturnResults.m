function [MDSResults] = MDSConditionsReturnResults(RDMs, userOptions, localOptions)
...
% RETURN VALUES
% MDSResults
%       The structure contains results of each MDS. For example, the first element
%       correspond to the first MDS results:
%       MDSResults(1).mdsCoordinates: the coordinates of the MDS solution.
%       mdsResults(1).dotNames: the name of each dot (corresponding to the
%                               mdsCoordinates).
%       MDSResults(1).dissimilarities: the dissimilarities of the raw data.
%       MDSResults(1).distances: the distances of the MDS solution.
%       MDSResults(1).disparities: the disparities of the MDS solution.
%       MDSResults(1).stress: the stress of the MDS solution.

% Cai Wingfield 3-2010, 5-2010, 6-2010
%
% EDITS
%         4-2018 Yun Shiuan: It returns the the results of the MDS
...
for RDMi = 1:nRDMs
	...
	[MDSResult]=figureMDSArrangementReturnResults(RDMs(RDMi), userOptions, veryLocalOptions);
    
    % Save the coordinates of the MDS to return
    RDMFieldName=matlab.lang.makeValidName(RDMs(RDMi).name); %Make a valid field name
    MDSResults.(RDMFieldName)=MDSResult;
