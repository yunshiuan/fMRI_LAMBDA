function [MDSResults] = figureMDSArrangementReturnResults(RDM, userOptions, localOptions)
...
% mdsResults
%       The MDS results.
%       mdsResults.mdsCoordinates: the coordinates of the MDS solution.
%       mdsResults.dotNames: the name of each dot (corresponding to the
%                             mdsCoordinates).
%       mdsResults.dissimilarities: the dissimilarities of the raw data.
%       mdsResults.distances: the distances of the MDS solution.
%       mdsResults.disparities: the disparities of the MDS solution.
%       mdsResults.stress: the stress of the MDS solution.

% EDITS
%         3-2010: CW: It now works with userOptions and localOptions, and some
%                     of the options fieldnames have been changed.
%         4-2018 Yun Shiuan: It returns the results of the MDS
%                     solution. This extension enables users to use the
%                     results for other purposes (e.g., get the pricise
%                     values of stress,distance or disparities, or re-plot 
%                     the MDS plots in other programs).
...
try
    ...
    error(['Fail at the initial MDS trial: ',char(description)]);
end
...
if isfield(localOptions,'figI_shepardPlots')&&~isempty(localOptions.figI_shepardPlots)
    ...
    distances=pdist(pats_mds_2D);
    shepardPlot(D,disparities,distances,localOptions.figI_shepardPlots,['\fontsize{12}MDS (' description{1} ')']);
end
%Save the MDS results (distances etc. in the square form)
MDSResults=struct('mdsCoordinates', pats_mds_2D,...
                  'dotNames',char(localOptions.textLabels{:}),...
                  'dissimilarities',D,...
                  'distances',squareRDMs(distances),'disparities',disparities,...
                  'stress',stress);
