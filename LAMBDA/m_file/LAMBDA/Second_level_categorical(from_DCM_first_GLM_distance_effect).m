...
path='D:\Yun-Shiuan_LAMBDA\DCM\Part0_First_GLM_for_DCM_multiple_runs_run_with_reslice'; % To get the subject list with valid runs (see below)
cd(path);
...
explicit_Mask={'D:\Yun-Shiuan_LAMBDA\template\for_this_study\binary_overlapped_WM_GM_common_mask_n40_with_signal_with_reslice.nii,1'};

for con=1:size(T_Parameters_name,2) %NOTE: con=1 refers to spmT_0002
    clear matlabbatch
    second_Result_Dir = strcat('D:\Yun-Shiuan_LAMBDA\DCM\Second_GLM_Reivew_Only\full_duration\',T_Parameters_name{con});
    mkdir(second_Result_Dir);
    matlabbatch{1}.spm.stats.factorial_design.dir = {second_Result_Dir};
    
    first_T_Contrast=strcat('D:\Yun-Shiuan_LAMBDA\DCM\Part0_First_GLM_for_DCM_multiple_runs_run_with_reslice\',...
...
for first_con=1:size(T_Parameters_name,2) 
    matlabbatch='';
    spm_File=strcat('D:\Yun-Shiuan_LAMBDA\DCM\Second_GLM_Reivew_Only\full_duration\',T_Parameters_name{first_con},'\SPM.mat');
    ...
    result_table_Dir=strcat('D:\Yun-Shiuan_LAMBDA\DCM\Second_GLM_Reivew_Only\full_duration\',T_Parameters_name{first_con},'\Result_table');
    mkdir(result_table_Dir);
    csv_Old_Name=ls(strcat('D:\Yun-Shiuan_LAMBDA\DCM\Second_GLM_Reivew_Only\full_duration\',T_Parameters_name{first_con},'\spm*.csv'));
    ...
    csv_Old_Full_Name=strcat('D:\Yun-Shiuan_LAMBDA\DCM\Second_GLM_Reivew_Only\full_duration\',T_Parameters_name{first_con},'\',csv_Old_Name);
